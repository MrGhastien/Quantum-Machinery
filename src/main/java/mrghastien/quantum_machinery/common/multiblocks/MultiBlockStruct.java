...
import mrghastien.quantum_machinery.util.math.Coord3;
...
public class MultiBlockStruct {
	...
	public MultiBlockStruct(Vector3i size, boolean optional) {
	...
	public Block get(Vector3i relPos) {
	...
	public MultiBlockStruct rotate(int angle) {
		...
		Vector3i tempSize = new Vector3i(tempPart.width, tempPart.height, tempPart.depth);
		Vector3i translator = null;
		Vector3i newTranslator = null;
		Vector3i newOrigin = new Vector3i(-(Math.abs((tempSize.getX() - 1) / 2)),
		...
		for (int x = newOrigin.getX(); x < newOrigin.getX() + tempSize.getX(); x++) {
			for (int y = newOrigin.getY(); y < newOrigin.getY() + tempSize.getY(); y++) {
				for (int z = newOrigin.getZ(); z < newOrigin.getZ() + tempSize.getZ(); z++) {
					...
					BlockPos patternPos = new BlockPos(MathHelper.rotate(newPos, angle / 90 * -90, new Vector3i(0, 1, 0)));
					if (newTranslator == null) {
						newTranslator = new Vector3i(-newOrigin.getX(), -newOrigin.getY(), -newOrigin.getZ());
					}
					if (translator == null) {
						translator = new Vector3i((Math.abs((width) / 2)),
	...
	public static Vector3i singleRotate(Vector3i index, MultiBlockStruct part, int angle) {
		MultiBlockStruct tempPart = part.getRotatedSize(angle);
		Vector3i tempSize = new Vector3i(tempPart.width, tempPart.height, tempPart.depth);
		Vector3i newTranslator = null;
		Vector3i newOrigin = new Vector3i(-(Math.abs((tempSize.getX() - 1) / 2)), -(Math.abs((tempSize.getY() - 1) / 2)), -(Math.abs((tempSize.getZ() - 1) / 2)));
		Vector3i translator = new Vector3i(-(Math.abs((part.width - 1) / 2)), -(Math.abs((part.height - 1) / 2)), -(Math.abs((part.depth - 1) / 2)));
		
		BlockPos patternPos = new BlockPos(index).add(translator);
		BlockPos newPos = new BlockPos(MathHelper.rotate(patternPos, angle / 90 * -90, new Vector3i(0, 1, 0)));
		if (newTranslator == null) {
			newTranslator = new Vector3i(-newOrigin.getX(), -newOrigin.getY(), -newOrigin.getZ());
		}
	...
	public MultiBlockStruct getRotatedSize(int angle) {
		return new MultiBlockStruct(MathHelper.abs(MathHelper.rotate(new Vector3i(width, height, depth), angle / 90 * 90, new Vector3i(0, 1, 0))), this.optional);
	}
	...
	public MultiBlockStruct getMergedSize(MultiBlockStruct part, int offsetX, int offsetY, int offsetZ) {
		...
		return new MultiBlockStruct(new Vector3i(totalWidth, totalHeight, totalDepth), this.optional);
	}
	
	public static  Map<MultiBlockStruct, Vector3i> singleMerge(Vector3i index,MultiBlockStruct part1, MultiBlockStruct part2, int offsetX, int offsetY, int offsetZ) {
		Map<MultiBlockStruct, Vector3i> result = new HashMap<>();
		...
		if (canBePart2[0] && canBePart2[1] && canBePart2[2]) {
			result.put(part2, new Vector3i(index.getX() - offsetX, index.getX() - offsetX, index.getX() - offsetX));
		} else {
			result.put(part1, new Vector3i(index.getX() - offsetX, index.getX() - offsetX, index.getX() - offsetX));
		}
		return result;
	}
