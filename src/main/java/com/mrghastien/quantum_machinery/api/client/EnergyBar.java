package com.mrghastien.quantum_machinery.api.client;
...
import com.mrghastien.quantum_machinery.util.RenderingUtils;
import com.mrghastien.quantum_machinery.util.RenderingUtils.GradientMode;
...
public class EnergyBar {
	...
	public List<String> getTooltip(BaseTile tile) {
		List<String> tooltips = new ArrayList<>();
		MachineEnergyStorage energy = getStorage();
		int transfer = energy.getInput() - energy.getOutput();
		tooltips.add("Energy: " + energy.getEnergyStored() + "/" + energy.getMaxEnergyStored() + Units.ENERGY);
		switch (type) {
			case STORAGE:
				tooltips.add((energy.getInput() > 0 ? TextFormatting.GREEN : TextFormatting.DARK_GRAY) + "Input : "
						+ energy.getInput() + Units.ENERGY + "/t");
				tooltips.add((energy.getOutput() > 0 ? TextFormatting.RED : TextFormatting.DARK_GRAY) + "Output : "
						+ energy.getOutput() + Units.ENERGY + "/t");
				tooltips.add((transfer > 0 ? TextFormatting.GREEN
			...
			case GENERATOR:
				if (tile instanceof GeneratorTile) {
					...
					tooltips.add(color + "Production : " + gen.getCurrentEnergyProduction() + Units.ENERGY + "/t");
					tooltips.add((energy.getOutput() > 0 ? TextFormatting.RED : TextFormatting.DARK_GRAY) + "Output : "
							+ energy.getOutput() + Units.ENERGY + "/t");
				}
			...
			case CONSUMER:
				if (tile instanceof MachineTile<?>) {
					...
					tooltips.add((energy.getInput() > 0 ? TextFormatting.GREEN : TextFormatting.DARK_GRAY) + "Input : "
							+ energy.getInput() + Units.ENERGY + "/t");
					tooltips.add(color + "Consumption : " + machine.getCurrentEnergyUsage() + Units.ENERGY + "/t");
				}
				break;
		}
		return tooltips;
