...
import static com.mrghastien.quantum_machinery.common.init.ModTileEntities.REFINERY;
...
public class RefineryTile extends MachineTile {
	...
	public RefineryTile() {
		super(REFINERY.get(), "container.refinery", 10000, 1000, 0);
		this.maxTimer = 120;
	}
		...
		if (!world.isRemote) {
			...
			if (this.hasEnergy()) {
				if (workTimer > 0) {
					...
					energy.consumeEnergy(toConsume, false);
