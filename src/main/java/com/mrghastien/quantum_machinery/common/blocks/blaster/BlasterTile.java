...
import com.mrghastien.quantum_machinery.common.capabilities.temperature.ITempHandler;
...
import com.mrghastien.quantum_machinery.common.items.BatteryStackHandler;
...
public class BlasterTile extends MachineTile {
	...
	public BlasterTile() {
		super(ModTileEntities.BLASTER.get(), "container.blaster.name", 50000, 0, 1024);
		this.maxTimer = 120;
	}
	...
	public void tick() {
		...
		if (workTimer < maxTimer && canProduce() && getProd() > 0) {
			...
			energy.generateEnergy(getProd(), false);
			markDirty();
		} else {
			isProducing = false;
		}
		...
		batteryHandler.ifPresent(h -> h.fillBatteries(energy.getLazy().cast()));
		BlockHelper.sendOutPower(this);
		super.tick();
	}

	private boolean canProduce() {
		return !energy.isFull() && (handler.map(h -> h.getStackInSlot(0)).orElse(ItemStack.EMPTY).getItem() == Items.COAL) || workTimer > 0;
	}
	...
	private int getProd() {
		return Math.min(energy.getAvailableSpace(), MAX_PROD);
	}
	...
	public <T> LazyOptional<T> getCapability(Capability<T> cap, Direction side) {
		...
		if (cap == CapabilityEnergy.ENERGY) {
			return CapabilityEnergy.ENERGY.orEmpty(cap, energy.getLazy().cast());
		}
